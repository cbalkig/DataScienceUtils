...
def seasonal_all(df, col_name, col_display, date_col_name, folder=None, orig_color="blue", trend_color="red",
                 legend_position="best", legend_bbox_to_anchor=None, x_label_rotation=0, period=2, is_trends=True,
                 is_seasonal=True, is_residual=True):
    temp_df = df.copy()
...
def trends(df, col_names, col_displays, date_col_name, folder=None, colors=None, legend_position="best",
           legend_bbox_to_anchor=None, x_label_rotation=0, period=2, method='plot', file_name_suffix=""):
    if colors is None:
    ...
    if folder is not None:
        if not os.path.exists(folder):
            os.mkdir(folder)
        fig.savefig(os.path.join(folder, "Trend_" + _clean_file_name(file_name[:100]) + ".png"))
